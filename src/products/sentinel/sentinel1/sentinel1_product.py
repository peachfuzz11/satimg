...
from transformers.gcp_transformer import GCPTransformer
...
class Sentinel1Product(Product):
    ...
    def view(self, image_slice: ImageSlice = None, a_min=0, a_max=510., *args, **kwargs) -> Image:
        arr = self.product
        if image_slice:
            arr = self.read_slice(image_slice)
        arr = (arr.clip(a_min, a_max) * 255. / a_max).astype("uint8")[1]
        image = PIL.Image.fromarray(arr.values, mode="L")
        return image
