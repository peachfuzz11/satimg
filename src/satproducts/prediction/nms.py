...
def iou(boxes1, boxes2):
    ...
    iou = intersection_area / (area_boxes1[:, None] + area_boxes2 - intersection_area)

    return iou

    x1 = boxes1[:, None, 0] - boxes1[:, 2] / 2
    y1 = boxes1[:, None, 1] - boxes1[:, 3] / 2
    x2 = boxes2[:, None, 0] - boxes2[:, 2] / 2
    y2 = boxes2[:, None, 1] - boxes2[:, 3] / 2

    intersection_x1 = numpy.maximum(x1, x2)
    intersection_y1 = numpy.maximum(y1, y2)
    intersection_x2 = numpy.minimum(x1 + boxes1[:, 2], x2 + boxes2[:, 2])
    intersection_y2 = numpy.minimum(y1 + boxes1[:, 3], y2 + boxes2[:, 3])

    intersection_area = numpy.maximum(intersection_x2 - intersection_x1, 0) * numpy.maximum(
        intersection_y2 - intersection_y1, 0)
    area_boxes1 = boxes1[:, 2] * boxes1[:, 3]
    area_boxes2 = boxes2[:, 2] * boxes2[:, 3]
